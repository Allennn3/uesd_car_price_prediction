...
torch.save(model, '../model/LR_model.pth')

val_results.to_csv('../New_Results/LR_predictions.csv', index=False)
loss_records.to_csv('../New_Results/LR_loss_records.csv', index=False)
...
loss_pd.to_csv('../New_Results/LR_loss_pd.csv', index=False)
...
plt.savefig('../New_Results/LR_loss.png')
plt.show()
writer.close()
